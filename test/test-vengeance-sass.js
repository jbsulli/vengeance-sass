...
describe('VengeanceSass', () => {
    ...
    it.only("should comple sass vars", () => {
        var sass = new Sass('/css/styles.css');
        var r = pug.render('block style\nm.test.vars\na(href="#") click me', withVengeance({ basedir: './test/pug' }, { styles:sass }));
        expect(r).to.equal('<link href="/css/styles.css" rel="stylesheet"/><a href="#">click me</a>');
        r = sass.compile();
        //expect(r.css.toString()).to.equal('body {\n  background: #000; }\n\n\n/*# sourceMappingURL=styles.css.map */');
        expect(JSON.parse(r.map.toString())).to.deep.equal({
            version: 3,
            sources: ["test/vars.md"],
            names: [],
            mappings: "AAIA;AAIA;AACA;AACA",
            file: "styles.css"
        });
    });
});